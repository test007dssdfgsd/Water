  ...
  <div class="admin-users">
    ...
    <div class="admin-users__hero">
      <div class="admin-users__hero-text">
        ...
        <div>
          <h1 class="admin-users__title">{{ $t('user') }} (Admin)</h1>
    ...
    <div class="admin-users__panel">
      ...
      <div v-if="!list.length" class="admin-users__empty">
        ...
        <p class="admin-users__empty-hint">Нажмите «{{ $t('add') }}», чтобы создать первого администратора</p>
        ...
        <table class="admin-users__table">
          ...
          <tbody>
            <tr v-for="row in list" :key="row.id">
              ...
              <td style="font-size: 12px;">{{ row.phone_number ? formatPhoneDisplay(row.phone_number) : '—' }}</td>
              <td>
...
export default {
  ...
  methods: {
    ...
    /** (XX) XXX-XXXX — 9 raqamgacha, inputPhone.vue / AdminCompanies bilan mos */
    ...
    async loadList () {
      try {
        this.loading = true
        const res = await fetch(this.api() + '/WaterAdminUsers')
        this.loading = false
      ...
      try {
        ...
        if (isEdit) {
          if (this.form.password) {
            ...
            const resAuthGet = await fetch(this.api() + '/WaterAdminAuths/' + this.form.authId)
        ...
        const resUser = await fetch(this.api() + '/WaterAdminUsers', {
          method: 'POST',
        ...
        const authBody = {
          ...
          user_type: 0,
          client_type_info: 0,
          waterAdminUserid: uid
        }
        const resAuth = await fetch(this.api() + '/WaterAdminAuths', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    async remove (row) {
      ...
      try {
        ...
        if (row.auth_id || (row.auth && row.auth.id)) {
          ...
          await fetch(this.api() + '/WaterAdminAuths/' + aid, { method: 'DELETE' })
        }
        const res = await fetch(this.api() + '/WaterAdminUsers/' + row.id, { method: 'DELETE' })
        this.loading = false
        if (res.status === 200 || res.status === 201) {
          await this.loadList()
        } else {
...
/* Modal body (slot is inside modal component — use :deep or unscoped block) */
</style>

<style lang="scss">
/* Modal slot is not inside scoped .admin-users — style modal-train body */
.admin-modal {
